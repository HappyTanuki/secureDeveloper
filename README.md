...
> 전체적으로 기능별로 분리하였고 uri 디렉토리와 메소드명을 디렉토리와 함수명에 사용하여 어떤 곳에 문제가 생겼을 시에 쉽게 찾아갈 수 있도록 하였음

> main.go:25
> 로거를 활용하여 기록 조회를 할 수 있도록 함.

> api/auth/auth.go:11
> 반복되는 쿠키 검증과 인증 로직을 hander 미들웨어로 분리

> api/auth/register/register.go:36
> 비밀번호를 저장할 때에 솔트를 쳐서 저장하도록 하여 보안성 향상과 법을 준수하려는 노력을 하였음

> consts/consts.go:3
> 파일들 전체에서 여러 번 쓰이는 const한 정보를 따로 분리하였음

> objects/db/db.go:1
> db 관련한 조작을 전부 한 파일로 분리하고 글로벌 변수 DB를 선언하여 편리하게 여러 파일에서 조회하기 쉽게 하였음

> session/session.go:49, 15
> 세션 또한 글로벌 변수로 선언하여 프로그램 전역에서 접근하기 쉽도록 하였음
> 또한 뮤텍스를 도입하여 여러 프로세스가 동시에 세션을 조회해도 깨지지 않고 데이터를 일고 쓸 수 있게 하였음

> util/util.go:1
> 유틸리티 프로그램을 분리하여 main.go가 지저분해지지 않도록 관리하였음

> 게시글 조회 삭제 등등은 미구현, 로그인, 회원가입 등 권한 관련한 부분은 전부 구현 완료
